...
function getLinearEquationRoot(a, b) {
  if (a === 0) {
    if (b === 0) {
      throw new Error('Infinite solutions: equation is always true');
    } else {
      throw new Error('No solution: equation is inconsistent');
    }
  }
  return -b / a;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const angle = Math.acos(Math.max(-1, Math.min(1, cosineTheta)));
